        ...
        <fieldset class="card mb-4">
            ...
            <div class="card-block">
                <div class="form-group">
                    <?php $val = isset($_POST['autocomplete-live']) ? htmlspecialchars($_POST['autocomplete-live']) : null ?>
                    <label class="form-control-label" for="autocomplete-live">Votre pays préféré :</label>
                    <input type="text"
                        id="autocomplete-live"
                        name="autocomplete-live"
                        class="form-control"
                        value="<?= $val ?>"
                        data-autocomplete="autocomplete.php"
                        data-autocomplete-default="<?= $val ?>"
                        data-autocomplete-params='<?= json_encode([
                            'test' => 'hey',
                        ]) ?>'
                        data-autocomplete-invalid="Veuillez sélectionner une valeur dans la liste."
                        data-autocomplete-empty="Aucun résultat"
                        data-autocomplete-loading="Chargement…">
                </div>
                <div class="form-group">
                    <?php $val = isset($_POST['autocomplete-static']) ? htmlspecialchars($_POST['autocomplete-static']) : null ?>
                    <label class="form-control-label" for="autocomplete-static">Votre fruit préféré :</label>
                    <select
                        id="autocomplete-static"
                        name="autocomplete-static"
                        class="form-control"
                        data-autocomplete="true"
                        data-autocomplete-input-class="form-control"
                        data-autocomplete-invalid="Veuillez sélectionner une valeur dans la liste."
                        data-autocomplete-empty="Aucun résultat"
                        data-autocomplete-loading="Chargement…">
        ...
        <fieldset class="card mb-4">
            ...
            <div class="card-block">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="birthday" class="form-control js-datepicker" id="birthday" placeholder="AAAA-MM-DD">
                </div>

                <div class="form-group">
                    <label class="form-control-label" for="event-date">Date de l'événement</label>
                    <input type="text" name="event-date" class="form-control js-datetimepicker" id="event-date" placeholder="AAAA-MM-DD hh:mm">
                </div>
            </div>
        </fieldset>
