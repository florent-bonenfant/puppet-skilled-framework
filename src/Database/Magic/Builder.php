...
class Builder
{
     ...
     *
     * @var \Globalis\PuppetSkilled\Database\Query\Builder
     */
    protected $query;
     ...
     * @param  \Globalis\PuppetSkilled\Database\Magic\Scope|\Closure  $scope
     * @return $this
     */
    ...
    public function withCount($relations)
    {
        ...
        foreach ($this->parseWithRelations($relations) as $name => $constraints) {
            ...
            unset($alias);
            if (count($segments) == 3 && mb_strtolower($segments[1]) == 'as') {
            ...
            $column = str_snake(isset($alias) ? $alias : $name).'_count';
     ...
     *
     * @param  string  $relation
     * @return \Globalis\PuppetSkilled\Database\Magic\Relations\Relation
     */
     ...
     * @param  mixed  $id
     * @param  array  $columns
     * @return \Globalis\PuppetSkilled\Database\Magic\Model|static[]|static|null
     */
    ...
    public function findOrFail($id, $columns = ['*'])
    {
        ...
        throw new RuntimeException("No query results for model [".get_class($this->model)."]");
    }
    ...
    {
        return $this->firstOrNew($attributes)->fill($values)->save();
    }
     ...
     *
     * @throws \Globalis\PuppetSkilled\Database\Magic\ModelNotFoundException
     */
     ...
     */
    public function firstOr($columns = ['*'], callable $callback = null)
    {
     ...
     * @return array
     */
    protected function eagerLoadRelation(array $models, $name, callable $constraints)
    {
    ...
    public function getRelation($name)
    {
        ...
        $relation = Relation::noConstraints(function () use ($name) {
            try {
                ...
                throw new RuntimeException("Call to undefined relationship [".$name."] on model [".get_class($this->getModel())."].");
            }
        });
    ...
    {
        return (mb_strpos($name, '.') !== false) && (substr($name, 0, strlen($relation.'.')) === $relation.'.');
    }
    ...
    protected function addUpdatedAtColumn(array $values)
    {
        ...
        if (null === array_get($values, $column)) {
            array_set($values, $column, $this->model->freshTimestampString());
        }
        return $values;
    }
    ...
    {
        $whereBooleans = array_pluck($whereSlice, 'boolean');

        // Here we'll check if the given subset of where clauses contains any "or"
        // booleans and in this case create a nested where expression. That way
        // we don't add any unnecessary nesting thus keeping the query clean.
        if (in_array('or', $whereBooleans)) {
            $query->wheres[] = $this->createNestedWhere($whereSlice, $whereBooleans[0]);
        } else {
    ...
    protected function parseWithRelations(array $relations)
    {
        ...
        foreach ($relations as $name => $constraints) {
            ...
            if (is_numeric($name)) {
                ...
                list($name, $constraints) = (mb_strpos($name, ':') !== false)
    ...
    {
        return array_get($this->localMacros, $name);
    }
