...
if (!function_exists('csrf_anchor')) {
     ...
     * @return   string
     */
    function csrf_anchor($uri = '', $title = '', $attributes = '', $as_array = false)
    {
        ...
        if ($CI->config->item('csrf_protection') === true) {
            if (!$as_array) {
                ...
                $form = '<form action="'.$siteUrl.'" name="form_'.$CI->security->get_csrf_token_name().'" method="post" class="d-inline" accept-charset="'.strtolower(config_item('charset')).'">';
                $form .= '<input type="hidden" name="'.$CI->security->get_csrf_token_name().'" value="'.html_escape($CI->security->get_csrf_hash()).'" />';
                $form .= '</form>';
                $input = '<button type="submit" '.$attributes.' form="form_'.$CI->security->get_csrf_token_name().'">'.$title.'</button>';
...
if (!function_exists('uri_is_active')) {
    function uri_is_active($uri)
    {
        ...
        if (preg_match('/^' .preg_quote($uri, '/'). '\b/i', $currentUri)) {
            return true;
        } else {
            return false;
        }
