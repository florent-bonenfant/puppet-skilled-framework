...
<?= $this->element(
    ...
    [
        'actions' => [
            [
                'uri' => 'backoffice/modules/cgv/add/',
    ...
    <div class="card-block">
        ...
        <form action="<?= site_url($baseUrl) ?>" method="get" accept-charset="utf-8" class="FilterForm">
            <div class="form-group">
                <label for="<?= $preprendId ?>label" class="form-label"><?= lang('cgv_label_filter_label'); ?></label>
                <input type="text" class="form-control" name="label" id="<?= $preprendId ?>label" value="<?= html_escape($filters->getValue('label')) ?>">
            </div>
            <div class="form-group">
              <label class="form-label"><?= lang('cgv_label_companies'); ?></label><br/>
              <?= $this->element(
            ...
            <div class="form-group">
              <label class="form-label"><?= lang('cgv_label_families'); ?></label><br/>
              <?= $this->element(
                  'form/checkbox_inline',
                  [
                      'name' => 'families[]',
    ...
    [
       ...
       'displayed_fields' => [
           ...
           [
               ...
               'label' => 'lang:cgv_label_publication_date',
               'formater' => function ($item) {
           ...
           [
               ...
               'label' => 'lang:cgv_label_end_publication_date',
               'formater' => function ($item) {
           ...
           [
               'query_key' => 'document',
               'class' => 'table-actions',
               'label' => 'lang:cgv_label_document',
               'formater' => function($item) {
                  if ($item->document) {
                    return '<a href="' . site_url('backoffice/modules/cgv/download/' . $item->getRouteKey()) . '" class="btn-success btn btn-sm" data-toggle="tooltip" data-placement="top" data-original-title="'.lang('general_action_download').'" target="_blank"><i class="material-icons">file_download</i></a>';
                  }
               }
           ],
           [
