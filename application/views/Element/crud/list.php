...
$this->element('crud/pagination', [
    'anchor' => $this->fetch('anchor'),
    'pager_result' => $pagerResult,
]);
...
foreach ($this->fetch('displayed_fields') as $value) :
    ...
    $label = $value['label'];
?>
            <th <?=(!empty($value['header_class']) ? 'class="' . $value['header_class'] . '"' : '') ?>>
<?php
if ($queryKey && ($urlKey = $pager->isSortable($queryKey))) :
    if ($pagerResult['sort'] == $queryKey) :
        if ($pagerResult['direction'] == 'desc') :
?>
<?= anchor($baseUrl . '?order=' . $queryKey . '&direction=asc' . $this->fetch('anchor'), lang_libelle($label) . ' <i class="material-icons">arrow_drop_down</i>', ['title' => lang_libelle($label)]) ?>
<?php
        else :
?>
<?= anchor($baseUrl . '?order=' . $queryKey . '&direction=desc' . $this->fetch('anchor'), lang_libelle($label) . ' <i class="material-icons">arrow_drop_up</i>', ['title' => lang_libelle($label)]) ?>
<?php
        endif;
    else :
?>
<?= anchor($baseUrl . '?order=' . $queryKey . $this->fetch('anchor'), lang_libelle($label) . ' <i class="material-icons">import_export</i>', ['title' => lang_libelle($label)]) ?>
<?php
    endif;
else :
?>
